...
**換日時間定案為台北午夜 00:00**（第 11.1 節設計決策，含推翻它的代價）。實作照這個做，不需要等任何人確認。
...
所以 `coffee-catalog` 把 `branchId` 當成不透明字串鍵。**如果實作中發現這條界線走不通，不要自己加依賴** —— 依 `AGENTS.md` 禁止事項第 2 條，把理由與你採用的替代做法寫進 PR 描述，由 Claude 在審查時裁決。不要停下來等回覆。

---

## 8a. 施工階段

本規格分三個階段，**每個階段獨立編譯、測試全綠、CI 綠、獨立可合併**。依 `AGENTS.md`「施工階段與中斷續作」，一次執行以推進**一個**階段為目標，階段完成就 commit + push —— push 是唯一能跨執行保存進度的動作。

三個階段可以放在同一支 `codex/g13-branch-availability` 分支、同一個 PR 逐步推進（推薦，審查時看得到完整脈絡）。**未完成的階段不得標記 ready for review，也不得啟用 auto-merge。** PR 描述維護進度檢查表：

```markdown
## 施工進度（G13）
- [x] S1 資料層與權限常數 —— 已完成，CI 綠
- [ ] S2 可用性維護 API —— 進行中：端點與稽核已完成，越權測試尚未補齊
- [ ] S3 菜單查詢與下單套用 —— 未開始
```
...
| 項目 | 內容 |
| --- | --- |
| 動到 | `db/migration/V{next}__branch_menu_availability.sql`（新增）、`Identity.PERMISSIONS`、`InitialData.java:39-45` |
| 規格章節 | 第 4 節、第 4.3 節 |
| 為什麼可獨立合併 | 只建表與 seed 權限，**沒有任何程式讀 `branch_products`**。沒有 Java API 簽章變更，既有行為與售價逐欄位不變 |

驗收子集：第 9 節「資料模型」整段。

> 權限常數與 migration 必須在**同一個 commit**：`Identity.PERMISSIONS` 少了 `MENU_AVAILABILITY`，`HQ` 角色（吃全集，`InitialData.java:46`）就拿不到，而 migration 已經把它塞進 `MANAGER` / `CASHIER` —— 兩邊會不一致。
...
| 項目 | 內容 |
| --- | --- |
| 動到 | `Catalog.Product` 加 `availability` 欄位、`list()` / `sellable()` 簽章變更、`CatalogService` 的三個 `new Product(...)` 呼叫點、`OrderService.create()` 的品項迴圈、`frontend/src/shared/types.ts`、`MenuView.vue`、`docs/API.md` |
| 規格章節 | 第 5.1 節、第 5.2 節的 `GET /api/menu`、第 6 節 |
| 為什麼不能再切 | `Product` 是 `record`、`sellable()` 是 interface 方法，兩者一改，`coffee-orders`、`coffee-app` 的既有測試與前端型別**同時**編譯失敗。必須在同一個 commit 才會綠 |

驗收子集：第 9 節其餘各段（「查詢」「下單」「前端」「規範」）與第 10.1 節。

**這是三個階段中最大的一個。** 一次執行做不完時，依 `AGENTS.md` 推一個能編譯的中間狀態並維持 draft，下次續作 —— 不要為了收尾把 S3 拆成半綠的兩半推上去。

> 有考慮過讓 `sellable()` 保留舊的單參數多載、S3 再切成兩半。**不採用**：留著不帶 `branchId` 的多載，等於留著一條「繞過分店可用性」的合法路徑，日後一定有人呼叫到。寧可有一個大階段，靠中斷續作機制處理。
...
依 `AGENTS.md`「設計決策的歸屬」（PO 於 2026-09-17 整批授權給 Claude），下列四項**已經定案，不是待決事項**。Codex 照著做，不需要等任何人回覆。每一項都附判斷理由與推翻它的代價，供下一輪推翻用。

### 11.1 營業日換日時間 —— **台北午夜 00:00**

`sold_out_date` 直接取 `LocalDate.now(ZoneId.of("Asia/Taipei"))`，沒有任何偏移計算（第 4.2 節的 `today()` helper）。

**理由**：`branches` 表現在連營業時間欄位都沒有（那是 G14 的範圍）。在營業時間模型存在之前，先發明一個「營業日起點」常數，等於在 G14 之外開一個平行的時間概念，G14 做的時候要收拾兩份。而午夜換日的好處是任何人查 `branch_products` 都看得懂 `20260917` 是什麼意思，不需要先知道某個隱藏的偏移量。

**代價**：營業到凌晨的分店，店員在 00:30 標的售完會在同一個班次內、30 分鐘後自動解除，要重標一次。三家分店目前沒有跨夜營業設定，所以現階段踩不到。

**推翻代價：低。** 改成 05:00 換日只要動 `today()` 一個 helper（`LocalDate.now(zone).minusHours(5)`），資料格式不變、已寫入的列不需要遷移。**G14（分店營業時間）落地時應該重新檢視這一條** —— 那時換日時間就該跟各分店的開店時間走，一併處理。

### 11.2 `CASHIER` 有 `MENU_AVAILABILITY` —— **給**

**理由**：售完是現場即時動作，店長不一定在場。不給的話，替代作法只剩「打電話請店長遠端標記」，或維持現狀「顧客點了才說沒有」—— 後者正是本規格要消滅的問題。風險有界：資料範圍是 `BRANCH`，最壞情況是店員誤標自店某品項售完，一個動作就能解除，而且每次設定都有稽核列（第 7 節）。誤標的損失是幾筆訂單，不給的損失是這個功能等於沒做。

**推翻代價：中，而且有時效。** 合併**之前**要收緊，只要拿掉 migration 與 `InitialData` 的 `CASHIER` 兩行；合併**之後**才要收緊，就得另開一個新版號的 migration 做 `DELETE FROM role_permissions`（`AGENTS.md` 禁止修改已存在的 migration 檔）。要推翻請在 G13 進主線前提出。

### 11.3 區域定價（分店各自售價）—— **排除本規格，另立 G18**

**編號注意：`G17` 已由 `G06` 第 13.6 節的「點餐 UI 常用組合快捷」占用**，區域定價用 **G18**。

**理由**：區域定價同時動到金額重算、成本快照與報表毛利三條路徑，而且與 G06 的選項加價撞在同一段計算裡。本規格的可用性是純布林狀態，**完全不碰金額**（第 6 節第 2 點），兩者混在一起會讓「後端一律依有效菜單重算」這條硬規則的審查範圍暴增。

**推翻代價：低（對本規格而言）。** `branch_products` 之後要加 `price_override` 是加法，不需要改本規格產生的任何一列資料。但它應該排在 G06 與 G13 都合併之後，不要並行。

### 11.4 `UNLISTED` 的既有訂單照常出現在歷史與報表 —— **是預期行為，不做任何處理**

**理由**：訂單品項是**快照**（`AGENTS.md` 金額規則：「建立訂單時快照名稱、分類、單價、成本、選項與數量；之後改菜單不回寫歷史」）。可用性回答的是「現在還能不能賣」，不是「過去有沒有賣過」。把 `UNLISTED` 的歷史訂單藏起來，當月營收就會對不上已經收到的現金 —— 那是比看到一個已停售品項嚴重得多的問題。

**推翻代價：高。** 等於推翻整個快照原則，會連帶影響 G01 對帳與報表。不建議。

---

## 12. 給 Codex 的施工提醒

- **開工前先確認 G06 的狀態。** 兩者都改 `Catalog.sellable()` 的簽章與 `OrderService` 的品項迴圈，同時進行一定會撞。衝突集中在本規格的 **S3**；S1 與 S2 與 G06 沒有交集，G06 還沒合併時也可以先推進這兩階
- **開工前先看第 8a 節的施工階段**，以及分支／PR 已經做到哪一階段。一次執行推進一個階段就夠
...
- **規格有錯或漏掉邊界條件：講出來，然後繼續做。** 寫進 PR 描述或 `docs/reports/`，說明你補了什麼、為什麼，缺口由下一輪規格修補（`AGENTS.md`「設計決策的歸屬」與禁止事項第 9 條）。不要停下來等回覆 —— 沒有人會回
