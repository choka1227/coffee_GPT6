...
**規格書有錯或不完整時，先說出來，不要自己補洞後默默實作。** 規格會有遺漏的邊界條件或技術上不可行的設計。發現了要明講 —— 寫在設計摘要與 PR 描述裡，需要時也寫進 `docs/reports/`。

但**講完就繼續做，不要停下來等回覆**（見「設計決策的歸屬」）。依你的判斷補上合理的處理方式，並在 PR 描述標明你補了什麼、為什麼。缺口由下一輪規格修補。要避免的是「默默補洞」，不是「補洞」。
...
## 設計決策的歸屬（PO 於 2026-09-17 決定）

**PO 已把設計決策整批授權給 Claude（PM / SA），不再人工介入設計。** 具體規則：

- 規格書裡的設計取捨由 Claude 定案，**不需要 PO 逐項確認**。規格書不再有「待 PO 決定」章節，改為「設計決策」並附判斷理由
- **Codex 不需要等待 PO 確認設計摘要**，看完規格書就可以開工
- 規格或實作出了問題，**由下一輪規格修補，不是停工等人**。發現問題 → 寫進 review 或 `docs/reports/` → Claude 在下一份規格書或修正規格書裡處理 → Codex 依新規格實作
- 這條授權**不擴及** `CLAUDE.md` 列出的那幾項：repo 設定、帳號與憑證、不可逆操作、`main` 分支。那些仍然要 PO 決定

這改變的是「誰決定」，不是「要不要想清楚」。決策仍然要寫下理由，讓下一輪有東西可以推翻。

---

## 工作流程

**第 1 步 — 設計摘要（不等確認，但要留下紀錄）**

輸出簡短設計摘要：要新增/修改哪些檔案、DB schema 變更與 migration 檔名、跨模組依賴的變化（特別點出是否踩到模組邊界）、規格書裡有問題或遺漏的地方、測試計畫。

**寫完直接進第 2 步，不要等 PO 回覆。** 摘要的用途是留下可追溯的設計紀錄，以及讓 Claude 在審查時對照，不是一道核准關卡。

規格書有錯或不完整時仍然要**先講出來**（寫在設計摘要與 PR 描述裡），但講完就繼續做，依你的判斷補上合理的處理方式並標明。不要為了等回覆而停下來。

**第 2 步 — 實作**

一次專注一個模組，依序交付。同步交付 migration 與測試，不要說「測試之後再補」。**依規格書的「施工階段」逐階段推進**，見下一節。
...
實作 agent 有額度與時間上限，**一份規格常常一次跑不完**。這一節處理「跑到一半沒額度了」要怎麼辦。核心原則只有一句：

> **寧可「一個階段完整且綠」，也不要「三個階段各做一半」。**

半成品合不進去、審不了、也接不下去，等於零。完整的一個階段則是永久的進度。

### 規格書會標示施工階段

每份規格書都有「施工階段」一節，把工作切成有順序的階段。每個階段的定義是：

- **獨立可交付** —— 自己就能編譯、測試全綠、CI 綠
- **獨立可合併** —— 單獨合進主線不會破壞任何既有行為
- **有自己的驗收子集** —— 不需要等後面的階段就能驗

沒有標示階段的規格，視為只有一個階段。

### 一次執行的目標是「推進一個階段」

1. 開工前先看規格的施工階段，以及既有分支／PR 已經做到哪一階段
2. **以完成「下一個未完成的階段」為本次目標**，不要貪多
3. 階段完成 → 本機跑 `frontend build` 與 `backend verify` → commit → **push**
4. 還有餘裕就接續下一個階段，同樣做完就推

**每個階段完成就推一次，不要整份規格做完才推。** push 是唯一能跨執行保存進度的動作 —— 本機工作區在執行結束後就沒了。

### 額度或時間不足時

察覺快要跑不完時，**立刻收尾，不要硬撐到被中斷**：

1. 把已完成、可編譯的部分 commit 並 **push 到分支**
2. PR 維持（或改為）**draft**
3. 在 PR 描述維護**進度檢查表**，標明每個階段的狀態
4. 回報時說明做到哪裡、下一步是什麼

**未完成的工作絕對不要：** 標記 ready for review、啟用 auto-merge、或讓它以任何方式進入可合併狀態。半成品進主線比沒有進度糟得多。

如果連一個階段都沒做完，**推一個「能編譯、測試綠、但功能尚未接上」的中間狀態**（例如只有 migration 與新的 api record，還沒有人呼叫）。這種狀態是安全的。真的無法讓它綠，就在 PR 描述明確寫出「目前 head 無法通過 CI，僅為進度保存」，並維持 draft。

### PR 描述的進度檢查表格式

```markdown
## 施工進度（G06）
- [x] S1 資料層與初始化 —— 已完成，CI 綠
- [ ] S2 Catalog 選項讀取與維護 —— 進行中：api record 與 migration 已完成，
      resolveOptions 驗證規則尚未實作
- [ ] S3 點餐整合與報表 —— 未開始
```

### 下一次執行：先續作，不要重開

開工前一律先找該規格是否已有 `codex/<主題>` 分支或 draft PR：

- **有** → 從那支分支繼續（先把主線 merge 進來解衝突），依進度檢查表接著做。**不要**另開新分支、不要重做已完成的階段
- **沒有** → 才從最新主線開新分支

### 給規格作者（Claude）的要求

- 每份規格都要切階段，**每階段的規模以「一次執行做得完」為準**
- 階段之間盡量用**加法**而不是破壞性變更：先加新的、讓新舊並存、最後才移除舊的。破壞性變更會逼出一個又大又不可分割的階段
- 每階段標註預估規模（動到哪些檔案）與該階段的驗收子集
- 階段太大就再切。切不開的話要在規格裡明講為什麼，讓實作端有心理準備
...
2. **不得改動模組邊界。** 要調整依賴關係先寫在設計摘要與 PR 描述裡明確提出，由 Claude 在審查時裁決
3. **不得引入新框架或新依賴。** 真的需要時寫明理由與替代方案，由 Claude 在審查時裁決 —— 預設是不引入
4. **不得在前端信任任何金額**
5. **不得把密鑰寫進程式碼或設定檔預設值**
6. **不得修改已存在的 Flyway migration 檔**
7. **不得為了讓測試通過而放寬測試。** 測試紅了是實作要改
8. **不得用英文寫終端使用者看得到的錯誤訊息**
9. **不確定不要默默猜。** 講出來（設計摘要、PR 描述、`docs/reports/`），寫明你採用了哪個做法與為什麼，然後繼續做。不要停下來等回覆 —— 沒有人會回。判斷錯了由下一輪規格修補，卡住不動則是什麼都沒有
